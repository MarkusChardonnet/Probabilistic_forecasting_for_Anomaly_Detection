...
def create_dataset(
    ...
    with open('{}data.npy'.format(path), 'wb') as f:
        np.save(f, stock_paths) # [nb_paths, dim, time_steps]
        np.save(f, observed_dates) # [nb_paths, time_steps]
        np.save(f, nb_obs)  # array of ints with size [nb_paths]
...
def CustomCollateFnGen(func_names=None, obs_perc = None, scaling_factor = 1):
    ...
    def custom_collate_fn(batch):
        ...
        if 'ad_labels' in batch[0]:
            ad_labels = np.concatenate([b['ad_labels'] for b in batch], axis=0)
        else: 
            ad_labels = None
...
def MicrobialCollateFnGen(func_names=None, use_only_dyn_ft_as_input=None,
    ...
    def microbial_collate_fn(batch):
        ...
        res = {'times': np.array(times), 'time_ptr': np.array(time_ptr),
               ...
               'true_mask': mask, 'ad_labels': ad_labels,
               'true_abx_exposure': abx_exposure,
