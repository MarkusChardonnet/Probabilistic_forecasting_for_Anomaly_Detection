...
LOSS_FUN_DICT = {
    ...
    'ad': compute_loss_ad,
    'ad_var': compute_loss_ad_variance,
    'ad_var_pos': compute_loss_ad_variance_bis,
}
...
class NJODE(torch.nn.Module):
    ...
    def custom_forward(self, times, time_ptr, X, obs_idx, delta_t, T, start_X,
        ...
        for i, obs_time in enumerate(times):
            # Propagation of the ODE until [steps] observations ahead
            ...
            c = 1
            while project_time < (fut_obs_time - 1e-10 * delta_t):  # 0.0001 delta_t used for numerical consistency.
                if project_time < fut_obs_time - delta_t:
                    delta_t_ = delta_t
                else:
                    delta_t_ = fut_obs_time - project_time
                if self.solver == 'euler':
                    proj_h, project_time = self.ode_step(
                        proj_h, delta_t_, project_time, last_X=last_X, tau=tau,
                        signature=c_sig)
                else:
                    raise NotImplementedError
                ...
                # path_h.append(proj_h)
                # path_y.append(self.readout_map(proj_h))
