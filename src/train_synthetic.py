...
def train(
    ...
    data_val = data_utils.IrregularDataset(
        model_name=dataset, time_id=dataset_id, idx=val_idx, obs_perc=val_data_perc)
    test_data_dict = None
    ...
    #scheduler = CyclicLR(optimizer, base_lr = learning_rate / 5., max_lr = learning_rate * 5, step_size_up = 200, mode = "exp_range")
            ...
            for j in range(dim):
                ...
                if plot_variance:
                    axs[j].fill_between(
                        ...
                        path_y_pred[:, i, j] + std_factor * path_std_2_pred[:, i, j],
                        label=model_name + " directly predicted standard deviation", color=std_color)
                if use_cond_exp:
                    axs[j].plot(path_t_true, path_y_true[:, i, j],
                    ...
                    if plot_variance:
                        ...
                        path_y_true[:, i, j] - std_factor * path_y_std_true[:, i, j],
                        path_y_true[:, i, j] + std_factor * path_y_std_true[:, i, j],
                        label='true conditional standard deviation', color=true_std_color)
